...
class Results(SimpleClass):
    ...
    def verbose(self):
        ...
        if boxes:
            for c in boxes.cls1.unique():
                n = (boxes.cls1 == c).sum()  # detections per class
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                c, conf, id = int(d.cls1), float(d.conf1), None if d.id is None else int(d.id.item())
...
class Boxes(BaseTensor):
    ...
    def __init__(self, boxes, orig_shape) -> None:
        ...
        ################################ Sửa lại cho fit với detect only + lấy 2 best class ################################
        n = boxes.shape[-1]
    ...
    def conf(self):
        ...
        ############################## Sửa lại nếu lấy 2 class có conf cao nhất ##############################
        # return self.data[:, -2] if len(self.data) == 6 else self.data[:, -4]
        return self.data[:, -2]
    ...
    def conf2(self):
        """Return the confidence values of the boxes."""
        ############################## Sửa lại nếu lấy 2 class có conf cao nhất ##############################
        return self.data[:, -2] if len(self.data) == 6 else self.data[:, -2]

    @property
    def cls(self):
        """Return the class values of the boxes."""
        # return self.data[:, -1] if len(self.data) == 6 else self.data[:, -3]
        return self.data[:, -1]
    
    @property
    def cls2(self):
        """Return the class values of the boxes."""
        return self.data[:, -1] if len(self.data) == 6 else self.data[:, -1]
